...
import com.farm.exam.utils.RoomSubjectLoadCacheUtils;
...
public class RoomController extends WebUtils {
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = roomServiceImpl.createRoomSimpleQuery(query)
					.search();
			result.runHandle(new ResultsHandle() {
				@Override
				public void handle(Map<String, Object> row) {
					if (!row.get("PSHOWTYPE").toString().equals("1")
							&& !row.get("PSHOWTYPE").toString().equals("2")) {
						row.put("TIMELEN", "-");
						row.put("SSORTTYPE", "-");
						row.put("OSORTTYPE", "-");
						row.put("COUNTTYPE", "-");
					}
					row.put("ANSERSNUM", roomServiceImpl
							.getRoomAnsersNum((String) row.get("ID")));
				}
			});
	...
	public Map<String, Object> examValid(String roomid, HttpSession session) {
		try {
			...
			for (Paper paper : papers) {
				...
				if (!paperServiceImpl.isAllHaveObjectiveSubjectAnswer(paper
						.getId())) {
					isWarning = true;
	...
	public Map<String, Object> editSubmit(Room entity, HttpSession session) {
		try {
			WcpLog.info("修改答题室[" + entity.getId() + "]:表单提交",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
			entity = roomServiceImpl.editRoomEntity(entity,
	...
	public Map<String, Object> addSubmit(Room entity, HttpSession session) {
		try {
			entity = roomServiceImpl.insertRoomEntity(entity,
					getCurrentUser(session));
			WcpLog.info("创建答题室[" + entity.getId() + "]:表单提交",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				...
				WcpLog.info("刪除答题室[" + id + "]:表单提交", getCurrentUser(session)
	...
	@ResponseBody
	public Map<String, Object> editRestartAble(String ids, Boolean ableType,
			HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				if (ableType != null) {
					roomServiceImpl.editRestartAble(id, ableType);
					WcpLog.info("修改答题室[" + id + "]:批量修改重复答题类型",
							getCurrentUser(session).getName(),
							getCurrentUser(session).getId());
				}
			}
	...
	public Map<String, Object> backup(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				...
				WcpLog.info("答题室状态变更[" + id + "]:数据归档", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public Map<String, Object> examPrivate(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				...
				WcpLog.info("答题室状态变更[" + id + "]:禁用", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public Map<String, Object> examPublic(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				...
				WcpLog.info("答题室状态变更[" + id + "]:发布", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public Map<String, Object> examtypeSetting(String ids, String examtypeId,
		...
		try {
			for (String id : parseIds(ids)) {
				roomServiceImpl.examTypeSetting(id, examtypeId,
						getCurrentUser(session));
				WcpLog.info("修改答题室[" + id + "]:设置分类", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public ModelAndView loadSubjectform(HttpSession session, String ids) {
		return ViewMode.getInstance().putAttr("ids", ids)
				.returnModelAndView("exam/RoomLoadSubsForm");
	}
	...
	public Map<String, Object> doLoadSubject(String ids, HttpSession session) {
		try {
			RoomSubjectLoadCacheUtils.doload(parseIds(ids), roomServiceImpl,
					paperServiceImpl, subjectServiceImpl);
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
			log.error(e.getMessage());
			return ViewMode.getInstance().setError(e.getMessage(), e)
	...
	public Map<String, Object> getLoadSubjectProcess(HttpSession session) {
		try {
			return ViewMode
					.getInstance()
					.putAttr("alls", RoomSubjectLoadCacheUtils.getAllNums())
					.putAttr("completes",
							RoomSubjectLoadCacheUtils.getCompleteNums())
					.putAttr("taskstate", RoomSubjectLoadCacheUtils.getState())
					.returnObjMode();
		} catch (Exception e) {
			log.error(e.getMessage());
			return ViewMode.getInstance().setError(e.getMessage(), e)
		...
		try {
			...
			if (StringUtils.isNotBlank(ids)) {
				...
				if (StringUtils.isNotBlank(room.getExamtypeid())) {
					examtype = examTypeServiceImpl.getExamtypeEntity(room
							.getExamtypeid());
				}
			}
			...
			case (0): {// 查看
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("entity", room).putAttr("examType", examtype)
						.returnModelAndView("exam/RoomForm");
			}
			case (1): {// 新增
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.returnModelAndView("exam/RoomForm");
			...
			case (2): {// 修改
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("examType", examtype).putAttr("entity", room)
						.returnModelAndView("exam/RoomForm");
			}
			default:
				break;
	...
	public ModelAndView editDoTime(String ids) {
		try {
			...
			return ViewMode.getInstance().putAttr("idlist", idlist)
					.putAttr("ids", ids)
	...
	@ResponseBody
	public Map<String, Object> doTimeSubmit(String ids, String timetype,
			String starttime, String endtime, HttpSession session) {
		try {
			for (String roomid : parseIds(ids)) {
				roomServiceImpl.editDoTimes(roomid, timetype, starttime,
						endtime);
				WcpLog.info("修改答题室[" + roomid + "]:批量修改答题时间",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().setOperate(OperateType.ADD)
	...
	public ModelAndView popinfo(RequestMode pageset, String ids) {
		try {
			...
			ExamType type = examTypeServiceImpl.getExamtypeEntity(room
					.getExamtypeid());
			// 查找房间的答题人
			...
			if (room.getWritetype().equals("0")) {
				...
				node.put("ROOMNAME",
						"<span class='feild_green'>" + room.getName()
								+ "</span>");
				examUsers.add(node);
			} else {
			...
			if (type.getMngpop().equals("1")) {
				...
				node.put("USERNAME", "<span class='feild_green'>所有人</span>");
				node.put("TYPENAME",
						"<span class='feild_green'>" + type.getName()
								+ "</span>");
				mngUsers.add(node);
			}
			if (type.getMngpop().equals("2")) {
				mngUsers = examTypeServiceImpl.getTypePopUsers(
						room.getExamtypeid(), "1");
			}
			if (type.getMngpop().equals("3")) {
				Map<String, Object> node = new HashMap<>();
				node.put("USERNAME", "<span class='feild_red'>未授权</span>");
				node.put("TYPENAME",
						"<span class='feild_red'>" + type.getName() + "</span>");
				mngUsers.add(node);
			}
			...
			if (type.getAdjudgepop().equals("2")) {
				adjudgeUsers = examTypeServiceImpl.getTypePopUsers(
						room.getExamtypeid(), "2");
			}
			...
			return ViewMode.getInstance().putAttr("examUsers", examUsers)
					.putAttr("mngUsers", mngUsers)
					.putAttr("adjudgeUsers", adjudgeUsers)
					.returnModelAndView("exam/RoomPop");
		} catch (Exception e) {
			return ViewMode.getInstance().setError(e + e.getMessage(), e)
					.returnModelAndView("exam/RoomPop");
		}
