...
import com.farm.exam.mapper.SubjectUserownMapper;
...
public class SubjectUserOwnServiceImpl implements SubjectUserOwnServiceInter {
    ...
    @Override
    @Transactional
    public void addFinishTestSubject(String subjectid, Boolean isRight,
                                     LoginUser user) {
    ...
    @Override
    @Transactional
    public void addFinishStandardSubject(String subjectid, Boolean isRight,
                                         LoginUser user) {
        if (user != null) {
            // 統計用戶答題
            examStatServiceImpl.addFinishSubjectNum(subjectid, isRight, user);
     ...
     *
     * @param subjectid
     * @param modeltype 1.历史答题,2:收藏,3:错题
     * @param cardId    可以为空
     * @param user
     * @return
     */
    private SubjectUserown insertSubjectuserownEntity(String subjectid,
        ...
        subjectuserownDaoImpl.deleteEntitys(DBRuleList.getInstance()
        ...
        subjectUserownMapper.insertEntity(userOwn);
        return userOwn;
    }
    ...
    public void deleteSubjectuserownEntity(String id, LoginUser user) {
        subjectUserownMapper.deleteEntity(id);
    }
    ...
    public SubjectUserown getSubjectuserownEntity(String id) {
        ...
        return subjectUserownMapper.getEntity(id);
    }
    ...
    public int getBookNum(String subjectId, String userid) {
        return subjectuserownDaoImpl.countEntitys(DBRuleList.getInstance()
    ...
    public boolean doBook(String subjectId, String userid) {
        boolean isBook = subjectuserownDaoImpl.countEntitys(DBRuleList
        ...
        if (isBook) {
            ...
            subjectuserownDaoImpl.deleteEntitys(DBRuleList.getInstance()
                    .add(new DBRule("MODELTYPE", "2", "="))
                    .add(new DBRule("SUBJECTID", subjectId, "="))
                    .add(new DBRule("CUSER", userid, "=")).toList());
            return false;
        } else {
            // 訂閲
            SubjectUserown userOwn = new SubjectUserown();
