...
public class ExamTypeController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(ExamTypeController.class);
	@Autowired
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = examTypeServiceImpl.createExamtypeSimpleQuery(
					query).search();
			result.runDictionary("1:可用,0:禁用", "STATE");
			result.runDictionary(
					"1:<span class=\"feild_green\">所有人</span>,2:<span class=\"feild_red\">指定人</span>,3:未授权",
					"MNGPOP");
			result.runDictionary(
					"1:<span class=\"feild_green\">所有人</span>,2:<span class=\"feild_red\">指定人</span>,3:未授权",
					"ADJUDGEPOP");
	...
	public Map<String, Object> editSubmit(ExamType entity, HttpSession session) {
		try {
			entity = examTypeServiceImpl.editExamtypeEntity(entity,
	...
	public Map<String, Object> addSubmit(ExamType entity, HttpSession session) {
		try {
			entity = examTypeServiceImpl.insertExamtypeEntity(entity,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				examTypeServiceImpl.deleteExamtypeEntity(id,
						getCurrentUser(session));
			}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr("entity",
								examTypeServiceImpl.getExamtypeEntity(ids))
						.returnModelAndView("exam/ExamtypeForm");
			}
			case (2): {// 修改
				ExamType type = examTypeServiceImpl.getExamtypeEntity(ids);
				parent = examTypeServiceImpl.getExamtypeEntity(type
						.getParentid());
				return ViewMode.getInstance().putAttr("pageset", pageset)
