...
public class MaterialController extends WebUtils {
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = materialServiceImpl.createMaterialSimpleQuery(
					query).search();
	...
	public Map<String, Object> editSubmit(Material entity, HttpSession session) {
		...
		try {
			entity = materialServiceImpl.editMaterialEntity(entity,
	...
	public Map<String, Object> addSubmit(Material entity, HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = materialServiceImpl.insertMaterialEntity(entity,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				materialServiceImpl.deleteMaterialEntity(id,
						getCurrentUser(session));
			}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr("entity",
								materialServiceImpl.getMaterialEntity(ids))
