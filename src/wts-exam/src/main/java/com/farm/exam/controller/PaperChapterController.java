...
public class PaperChapterController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(PaperChapterController.class);
	@Autowired
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = paperChapterServiceImpl
					.createPaperchapterSimpleQuery(query).search();
	...
	public Object chapterTypeTree(String id, String paperId) {
		try {
			if (id == null) {
				...
				EasyUiTreeNode nodes = new EasyUiTreeNode("NONE",
						paper.getName(), "open", "icon-customers");
				nodes.setChildren(EasyUiTreeNode.formatAsyncAjaxTree(
						EasyUiTreeNode.queryTreeNodeOne(
								id,
								"SORT",
								"(SELECT * from WTS_PAPER_CHAPTER where PAPERID='"
										+ paper.getId() + "')", "ID",
								"PARENTID", "NAME", "STYPE").getResultList(),
						EasyUiTreeNode.queryTreeNodeTow(
								id,
								"SORT",
								"(SELECT * from WTS_PAPER_CHAPTER where PAPERID='"
										+ paper.getId() + "')", "ID",
			...
			return EasyUiTreeNode.formatAsyncAjaxTree(
					EasyUiTreeNode.queryTreeNodeOne(id, "SORT",
							"WTS_PAPER_CHAPTER", "ID", "PARENTID", "NAME",
							"STYPE").getResultList(),
					EasyUiTreeNode.queryTreeNodeTow(id, "SORT",
							"WTS_PAPER_CHAPTER", "ID", "PARENTID", "NAME",
							"STYPE").getResultList(), "PARENTID", "ID", "NAME",
					"STYPE");
		} catch (Exception e) {
			log.error(e.getMessage());
			return ViewMode.getInstance().setError(e.getMessage(), e)
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Map<String, Object> editSubmit(PaperChapter entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = paperChapterServiceImpl.editPaperchapterEntity(entity,
					getCurrentUser(session));
			WcpLog.info("修改答卷[" + entity.getPaperid() + "]:章节修改",
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(PaperChapter entity,
			HttpSession session) {
		try {
			entity = paperChapterServiceImpl.insertPaperchapterEntity(entity,
					getCurrentUser(session));
			WcpLog.info("修改答卷[" + entity.getPaperid() + "]:章节創建",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				PaperChapter chapter = paperChapterServiceImpl
						.getPaperchapterEntity(id);
				WcpLog.info("修改答卷[" + chapter.getPaperid() + "]:章节刪除",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				paperChapterServiceImpl.deletePaperchapterEntity(id,
						getCurrentUser(session));
			}
	...
	@ResponseBody
	public Map<String, Object> sortUp(String paperSubjectIds,
			HttpSession session) {
		try {
			for (String id : parseIds(paperSubjectIds)) {

				paperChapterServiceImpl.subjectSortUp(id,
						getCurrentUser(session));
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public ModelAndView pointSetPage(String paperSubjectIds, HttpSession session) {
		...
		return ViewMode.getInstance()
				.putAttr("paperSubjectIds", paperSubjectIds)
				.putAttr("lenght", idlist.size()).putAttr("pageset", pageset)
				.returnModelAndView("exam/PaperChapterPointSetting");
	}

	@RequestMapping("/list")
	public ModelAndView index(String paperId, HttpSession session) {
		Paper paper = paperServiceImpl.getPaperEntity(paperId);
		return ViewMode.getInstance().putAttr("paperId", paperId)
				.putAttr("paper", paper)
	 ...
	 */
	@RequestMapping("/form")
	public ModelAndView view(RequestMode pageset, String parentid,
			String paperid, String ids) {
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								paperChapterServiceImpl
										.getPaperchapterEntity(ids))
			...
			case (1): {// 新增
				...
				return ViewMode.getInstance().putAttr("parentName", parentName)
						.putAttr("entity", chapter).putAttr("pageset", pageset)
						.returnModelAndView("exam/PaperchapterForm");
			}
			case (2): {// 修改
				PaperChapter chapter = paperChapterServiceImpl
		...
		String parentName = null;
		{
			if (parentId != null) {
				if (parentId.equals("NONE")) {
					...
					PaperChapter parent = paperChapterServiceImpl
							.getPaperchapterEntity(parentId);
					if (parent != null) {
	...
	@ResponseBody
	public Map<String, Object> addSubjectToPaper(String ids, String chapterId,
			HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				paperChapterServiceImpl.addSubject(id, chapterId,
						getCurrentUser(session));
			}
			PaperChapter chapter = paperChapterServiceImpl
					.getPaperchapterEntity(chapterId);
			WcpLog.info("修改答卷[" + chapter.getPaperid() + "]:添加题目",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
