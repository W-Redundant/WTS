...
public class JudgeHandle implements SubjectTipHandle {
	...
	public SubjectUnit expressTextSubject(String text, String subjectTypeId,
		...
		unit = subjectServer.initSubjectUnit(TipType.Judge, subjectTypeId,
				currentUser);
	...
	public boolean isHaveAnswer(List<CardAnswer> answers) {
		for (CardAnswer answer : answers) {
			if (StringUtils.isNotBlank(answer.getValstr())
					&& answer.getValstr().toUpperCase().equals("TRUE")) {
				return true;
			}
		}
		return false;
	...
	public void loadVal(SubjectUnit nuit, List<CardAnswer> list) {
		if (list.size() > 0) {
			...
			for (CardAnswer node : list) {
				if (node.getCtime().compareTo(choosecTime) > 0
						&& node.getValstr().equals("true")) {
	...
	public int runPointWeight(SubjectUnit unit) {
		for (AnswerUnit node : unit.getAnswers()) {
			if (node.getAnswer().getRightanswer().equals("1")) {
				if (node.getVal() != null
						&& node.getVal().toUpperCase().equals("TRUE")) {
					return 100;
				}
			}
		}
