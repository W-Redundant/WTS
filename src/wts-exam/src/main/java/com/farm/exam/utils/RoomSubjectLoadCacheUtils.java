...
public class RoomSubjectLoadCacheUtils {
	...
	public static void doload(final List<String> roomids,
			final RoomServiceInter roomServiceImpl,
			final PaperServiceInter paperServiceImpl,
			final SubjectServiceInter subjectServiceImpl) {
		if (state == 1) {
			...
			public void run() {
				try {
					...
					for (String roomid : roomids) {
						completeRoom++;
						for (Paper paper : roomServiceImpl
								.getLivePapers(roomid)) {
							List<String> subjectVersionids = paperServiceImpl
									.getAllSubjectVersionids(paper.getId());
							for (String subjectVersionid : subjectVersionids) {
								subjectServiceImpl
										.getSubjectUnit(subjectVersionid);
							}
						}
					}
				} finally {
