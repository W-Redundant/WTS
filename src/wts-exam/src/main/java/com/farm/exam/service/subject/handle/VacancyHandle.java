package com.farm.exam.service.subject.handle;
...
public class VacancyHandle implements SubjectTipHandle {
	...
	@Override
	public SubjectUnit expressTextSubject(String text, String subjectTypeId,
			LoginUser currentUser, SubjectServiceInter subjectServer,
			SubjectAnswerDaoInter subjectanswerDaoImpl,
			SubjectDaoInter subjectDaoImpl,
		...
		{// ----预处理
			subjectText = text.replaceAll("\\[ANS:\\S+?\\]", "____")
					.replaceAll("\\[SUB:\\S+题\\]", "");
			unit = subjectServer.initSubjectUnit(TipType.Vacancy,
					subjectTypeId, currentUser);
		}
		{// ----答案部分--------------------------------------------------------------
	...
	public int runPointWeight(SubjectUnit unit) {
		...
		for (AnswerUnit node : unit.getAnswers()) {
			if (node.getAnswer().getPointweight() != null) {
				...
				List<String> sameRightAnswer = Arrays.asList(node.getAnswer()
						.getAnswer().toUpperCase().split("\\|"));
				if (node.getVal() != null) {
					String userAnser = node.getVal().toUpperCase()
							.replaceAll("\\s*", "");
					// 用户答案 是正确答案
					if (sameRightAnswer.contains(userAnser)) {
						userRightNum = userRightNum
								+ node.getAnswer().getPointweight();
					} else {
						isAllRight = false;
					}
				} else {
