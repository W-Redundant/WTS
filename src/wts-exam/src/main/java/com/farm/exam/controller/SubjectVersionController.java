...
public class SubjectVersionController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectVersionController.class);
	@Autowired
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = subjectVersionServiceImpl
					.createSubjectversionSimpleQuery(query).search();
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(SubjectVersion entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = subjectVersionServiceImpl.insertSubjectversionEntity(
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectVersionServiceImpl.deleteSubjectversionEntity(id,
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								subjectVersionServiceImpl
										.getSubjectversionEntity(ids))
