...
public class SubjectPopController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectPopController.class);
	@Autowired
	...
	@ResponseBody
	public Map<String, Object> queryall(String functype, String typeids,
			DataQuery query, HttpServletRequest request) {
		try {
			...
			DataResult result = subjectPopServiceImpl
					.createSubjectpopSimpleQuery(query).search();
	...
	public Map<String, Object> editSubmit(SubjectPop entity, HttpSession session) {
		...
		try {
			entity = subjectPopServiceImpl.editSubjectpopEntity(entity,
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(String userids, String functype,
			String typeids, HttpSession session) {
		try {
			List<String> userIds = parseIds(userids);
			List<String> typeIds = parseIds(typeids);
			subjectPopServiceImpl.insertPop(userIds, typeIds, functype,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectPopServiceImpl.deleteSubjectpopEntity(id,
						getCurrentUser(session));
			}
	...
	@RequestMapping("/list")
	public ModelAndView index(String functype, String typeids,
			HttpSession session) {
		return ViewMode.getInstance().putAttr("functype", functype)
				.putAttr("typeids", typeids)
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr("entity",
								subjectPopServiceImpl.getSubjectpopEntity(ids))
