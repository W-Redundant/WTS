...
public class SubjectCommentController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectCommentController.class);
	@Autowired
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = SubjectCommentServiceImpl
					.createSubjectCommentSimpleQuery(query).search();
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				SubjectCommentServiceImpl.deleteSubjectCommentEntity(id,
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								SubjectCommentServiceImpl
										.getSubjectCommentEntity(ids))
