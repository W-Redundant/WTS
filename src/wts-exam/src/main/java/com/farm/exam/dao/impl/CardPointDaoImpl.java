...
import com.farm.exam.domain.CardPoint;
 ...
 */
@Repository
public class CardPointDaoImpl extends HibernateSQLTools<CardPoint> implements
		CardPointDaoInter {
	@Autowired
	...
	public List<Map<String, Object>> getAllRoomCardPoints(String roomid) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("SELECT B.POINT AS POINT,B.MPOINT AS MPOINT,C.SUBJECTID AS SUBJECTID,A.USERID AS USERID FROM WTS_CARD A LEFT JOIN WTS_CARD_POINT B ON A.ID=B.CARDID LEFT JOIN WTS_SUBJECT_VERSION C ON C.ID=B.VERSIONID "
