...
public class InterlocutionHandle implements SubjectTipHandle {
	...
	public SubjectUnit expressTextSubject(String text, String subjectTypeId,
		...
		unit = subjectServer.initSubjectUnit(TipType.Interlocution,
	...
	public int runPointWeight(SubjectUnit unit) {
		...
		for (AnswerUnit node : unit.getAnswers()) {
			if (node.getAnswer().getPointweight() != null) {
				...
				// --------------------------
				if (StringUtils.isNotBlank(node.getAnswer().getAnswer()
						.toUpperCase())) {
					sameRightAnswer = Arrays.asList(node.getAnswer()
