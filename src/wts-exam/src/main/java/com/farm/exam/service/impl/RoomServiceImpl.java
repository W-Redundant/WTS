...
import com.farm.exam.dao.CardPointDaoInter;
import com.farm.exam.dao.RoomDaoInter;
import com.farm.exam.dao.RoomUserDaoInter;
...
public class RoomServiceImpl implements RoomServiceInter {
	...
	public Room insertRoomEntity(Room entity, LoginUser user) {
		...
		farmFileManagerImpl.submitFile(entity.getImgid(),
				FILE_APPLICATION_TYPE.ROOMNOTE.getValue(), entity.getId());
		return entity;
	}
	...
	public Room editRoomEntity(Room entity, LoginUser user) {
		...
		farmFileManagerImpl.submitFile(entity2.getImgid(),
				FILE_APPLICATION_TYPE.ROOMIMG.getValue(), entity2.getId());
		return entity2;
	}
	...
	public RoomUnit getRoomUnit(String roomid, LoginUser currentUser,
		...
		if (entity.getPshowtype().equals("2") && isShuffle) {
			// 随机抽卷
			// 获取用户在房间内做过的答卷id
			List<String> paperids = cardServiceImpl.getUserPaperidsByRoom(
			...
			if (paperids == null || paperids.size() <= 0) {
				...
				{// 获得随机答卷
					// 用用户id获得一个数字
					User user = userServiceImpl.getUserEntity(currentUser
							.getId());
					int orderId = 100;
					if (user == null) {
						// 随机人员通过随机数获得随机答卷
						...
						// 登陆用户通过用户登陆时间获得随机数
						try {
							orderId = Integer.valueOf(user.getLogintime()
									.hashCode());
						} catch (Exception e) {
							e.printStackTrace();
							orderId = (int) (Math.random() * (100 - 1) + 1);
			...
			{// 可通过配置作弊参数，固定答题室中得随机答卷
				String cheatPaperId = CheatUtils.getOnlyCheatPaper(roomid,
						allPaperSet);
		...
		for (RoomPaper paper : papers) {
			PaperUnit paperUnit = paperServiceImpl.getPaperUnit(paper
					.getPaperid());
			paperUnit.setRoomPaper(paper);
			if (currentUser != null
					&& (entity.getPshowtype().equals("2") || entity
							.getPshowtype().equals("1"))) {
				Card card = cardServiceImpl.loadCard(paper.getPaperid(),
						roomid, currentUser.getId());
				if (card != null) {
			...
			if (entity.getPshowtype().equals("2") && isShuffle) {
				// 随机抽题，只取一套卷
				if (onlyPaperId != null
						&& onlyPaperId.equals(paper.getPaperid())) {
	...
	public boolean isLiveTimeRoom(Room room) {
		...
		// 2018-11-27 10:40
		String startTime = room.getStarttime().replaceAll(" ", "")
				.replaceAll("-", "").replaceAll(":", "");
		String endTime = room.getEndtime().replaceAll(" ", "")
				.replaceAll("-", "").replaceAll(":", "");
	...
	public boolean isHaveAdjudger(String roomid) {
		Room room = getRoomEntity(roomid);
		if (examTypeServiceImpl.getTypePopUsers(room.getExamtypeid(), "2")
				.size() > 0) {
			return true;
		}
		return false;
	...
	private void disorganizeSubjects(List<ChapterUnit> chapters) {
		for (ChapterUnit chapter : chapters) {
			if (chapter.getSubjects() != null
					&& chapter.getSubjects().size() > 0) {
				Collections.shuffle(chapter.getSubjects());
			}
			if (chapter.getChapters() != null
					&& chapter.getChapters().size() > 0) {
	...
	public void editState(String id, String state, LoginUser currentUser) {
		...
		if (state.equals("0")) {
			...
			if (!entity2.getPstate().equals("2")
					&& !entity2.getPstate().equals("3")) {
				throw new RuntimeException("只有发布和结束状态可停用!");
			}
		}
		if (state.equals("2")) {
			// 2发布(新建，停用,歸檔可发布)
			if (!entity2.getPstate().equals("1")
					&& !entity2.getPstate().equals("4")
	...
	@Override
	@Transactional
	public void backupRoom(String roomid, LoginUser currentUser)
			throws Exception {
		// 归档状态
		...
		// 将用户答题信息进行统计汇总 POINT,MPOINT,SUBJECTID,USERID
		{
			...
			for (Map<String, Object> node : cardPoints) {
				try {// 统计答题记录，题记录和用户记录
					...
					if (subjectid != null) {
						subjectUserOwnServiceImpl.addFinishStandardSubject(
								subjectid,
								(point == mpoint && mpoint > 0),
								FarmAuthorityService.getInstance().getUserById(
										(String) node.get("USERID")));
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
