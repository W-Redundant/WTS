...
import com.farm.exam.mapper.SubjectTypeMapper;
...
public class SubjectTypeServiceImpl implements SubjectTypeServiceInter {
	...
	@Override
	@Transactional
	public SubjectType insertSubjecttypeEntity(SubjectType entity,
			LoginUser user) {
	...
	public void deleteSubjecttypeEntity(String id, LoginUser user) {
		{
			List<SubjectType> subtypes = subjecttypeDaoImpl
					.selectEntitys(DBRuleList.getInstance()
							.add(new DBRule("PARENTID", id, "=")).toList());
			if (subtypes.size() > 0) {
				throw new RuntimeException("分类下还有分类，请先删除子分类！");
			}
		}
		{
			List<Subject> ableSubjects = subjectDaoImpl
					.selectEntitys(DBRuleList.getInstance()
							.add(new DBRule("PSTATE", "1", "="))
		...
		}
		{
			List<Subject> subjects = subjectDaoImpl.selectEntitys(DBRuleList
					.getInstance().add(new DBRule("PSTATE", "0", "="))
					.add(new DBRule("TYPEID", id, "=")).toList());
			// 删除所有临时试题
