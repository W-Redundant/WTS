...
public class RandomItemController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(RandomItemController.class);
	@Autowired
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = randomItemServiceImpl
					.createRandomitemSimpleQuery(query).search();
	...
	public Map<String, Object> editSubmit(RandomItem entity, HttpSession session) {
		...
		try {
			entity = randomItemServiceImpl.editRandomitemEntity(entity,
	...
	public Map<String, Object> addSubmit(RandomItem entity, HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = randomItemServiceImpl.insertRandomitemEntity(entity,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				randomItemServiceImpl.deleteRandomitemEntity(id,
						getCurrentUser(session));
			}
	...
	public ModelAndView index(HttpSession session) {
		return ViewMode.getInstance().returnModelAndView(
				"exam/RandomitemResult");
	}

	@RequestMapping("/steps")
	public ModelAndView steps(String itemids, HttpSession session) {
		return ViewMode.getInstance().putAttr("itemids", itemids)
				.returnModelAndView("exam/RandomstepResult");
	}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr("entity",
								randomItemServiceImpl.getRandomitemEntity(ids))
