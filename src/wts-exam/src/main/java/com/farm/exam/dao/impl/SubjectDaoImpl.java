 ...
 */
@Repository
public class SubjectDaoImpl extends HibernateSQLTools<Subject> implements
		SubjectDaoInter {
	@Autowired
	...
	public List<Subject> getSubjectsByPaperId(String paperId) {
		...
		List<Subject> list = (List<Subject>) sqlquery.addEntity(Subject.class)
				.list();
		return list;
	}
	...
	public String getIdByUuid(String uuid) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select ID from WTS_SUBJECT where uuid=?");
