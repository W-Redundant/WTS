 ...
 */
@Repository
public class PaperSubjectDaoImpl extends HibernateSQLTools<PaperSubject>
		implements PaperSubjectDaoInter {
	@Autowired
	...
	public int countPaperSubjectNum(String paperid) {
		return countEntitys(DBRuleList.getInstance()
	...
	public List<String> getAllSubjectVersionids(String paperid) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select VERSIONID from WTS_PAPER_SUBJECT where PAPERID=?");
