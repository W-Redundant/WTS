...
public class RoomPaperController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(RoomPaperController.class);
	@Resource
	...
	@ResponseBody
	public Map<String, Object> queryall(DataQuery query, final String roomid,
			HttpServletRequest request) {
		try {
			query = EasyUiUtils.formatGridQuery(request, query);
			query.addRule(new DBRule("ROOMID", roomid, "="));
			DataResult result = roomPaperServiceImpl
					.createRoompaperSimpleQuery(query).search();
			result.runHandle(new ResultsHandle() {
				@Override
				public void handle(Map<String, Object> row) {
					PaperUnit paper = cardServiceImpl.getRoomPaperUserNums(
							roomid, (String) row.get("PAPERID"));
	...
	public Map<String, Object> editSubmit(RoomPaper entity, HttpSession session) {
		...
		try {
			entity = roomPaperServiceImpl.editRoompaperEntity(entity,
					getCurrentUser(session));
			WcpLog.info("修改答题室[" + entity.getRoomid() + "]:表单修改答卷",
	...
	@ResponseBody
	public Map<String, Object> editOtherName(String ids, String name,
			HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				roomPaperServiceImpl.editOtherName(id, name,
						getCurrentUser(session));
			}
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.returnObjMode();
	...
	public Map<String, Object> clearOtherName(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				roomPaperServiceImpl
						.clearOtherName(id, getCurrentUser(session));
			}
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.returnObjMode();
		} catch (Exception e) {
	...
	public Map<String, Object> addSubmit(RoomPaper entity, HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = roomPaperServiceImpl.insertRoompaperEntity(entity,
					getCurrentUser(session));
			WcpLog.info("修改答题室[" + entity.getRoomid() + "]:表单添加答卷",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				RoomPaper roomPaper = roomPaperServiceImpl
						.getRoompaperEntity(id);
				WcpLog.info("修改答题室[" + roomPaper.getRoomid() + "]:刪除答卷",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				roomPaperServiceImpl.deleteRoompaperEntity(id,
						getCurrentUser(session));
			}
	...
	public Map<String, Object> clearUserCard(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				RoomPaper roomPaper = roomPaperServiceImpl
						.getRoompaperEntity(id);
				cardServiceImpl.clearRoomCard(roomPaper.getRoomid(),
						roomPaper.getPaperid(), getCurrentUser(session));
				WcpLog.info("修改答题室[" + roomPaper.getRoomid() + "]:清空试卷的所有答题卡",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
	...
	public ModelAndView index(String roomid, HttpSession session) {
		return ViewMode.getInstance().putAttr("roomid", roomid)
				.returnModelAndView("exam/RoompaperResult");
	}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr("entity",
								roomPaperServiceImpl.getRoompaperEntity(ids))
	...
	@ResponseBody
	public Object addPaperSubmit(String roomid, String paperids,
			HttpSession session) {
		try {
			for (String paperid : parseIds(paperids)) {
				roomPaperServiceImpl.addRoomPaper(roomid, paperid,
						getCurrentUser(session));
				WcpLog.info("修改答题室[" + roomid + "]:添加答卷",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
			}
			return ViewMode.getInstance().returnObjMode();
