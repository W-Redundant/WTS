...
public class CardQueryController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(CardQueryController.class);
	@Resource
		...
		try {
			{
				...
				String endT = null;
				if (statTime != null) {
					startT = StringUtils.isNotBlank(statTime.getValue()) ? statTime
							.getValue() : null;
				}
				if (endTime != null) {
					endT = StringUtils.isNotBlank(endTime.getValue()) ? endTime
							.getValue() : null;
				}
				if (startT != null) {
			...
			}
			{
				...
				if (orgid != null) {
					Organization org = organizationServiceImpl
							.getOrganizationEntity(orgid.getValue());
					if (org != null) {
						query.addRule(new DBRule("c.TREECODE", org
								.getTreecode(), "like-"));
					}
				}
			}
			query = EasyUiUtils.formatGridQuery(request, query);
			DataResult result = cardHisServiceImpl.createHisQuery(query)
					.search();
			result.runformatTime("STARTTIME", "yyyy-MM-dd HH:mm");
			result.runDictionary(
					"1:开始答题,2:手动交卷,3:超时未交卷,4:超时自动交卷,5:自动阅卷,6:完成阅卷,7:发布成绩",
					"PSTATE");
	...
	@ResponseBody
	public Map<String, Object> liveCardInfo(String id,
			HttpServletRequest request) {
		try {
			Card card = cardServiceImpl.getCardEntity(id);
			if (card == null) {
				return ViewMode.getInstance().putAttr("info", "当前答题卡不存在！")
						.returnObjMode();
			}
			int completenum = cardServiceImpl.getCardCompleteNum(card.getId());
			Paper paper = paperServiceImpl.getPaperEntity(card.getPaperid());
			String msg = "开始答题时间："
					+ TimeTool.getFormatTimeDate12(card.getStarttime(),
							"yyyy-MM-dd HH:mm:ss")
					+ "，结束答题时间："
					+ TimeTool.getFormatTimeDate12(card.getEndtime(),
							" HH:mm:ss") + "，完成题目数量：" + completenum + "/"
					+ paper.getSubjectnum();
	...
	@ResponseBody
	public Map<String, Object> liveQuery(DataQuery query,
			HttpServletRequest request) {
		try {
			{
				// 处理时间查询
			...
			query = EasyUiUtils.formatGridQuery(request, query);
			DataResult result = cardHisServiceImpl.createLiveQuery(query)
					.search();
			result.runformatTime("STARTTIME", "yyyy-MM-dd HH:mm");
			result.runDictionary(
					"1:开始答题,2:手动交卷,3:超时未交卷,4:超时自动交卷,5:自动阅卷,6:完成阅卷,7:发布成绩",
	...
	public void exportHisExcel(String ruleText, HttpServletRequest request,
		...
		try {
			...
			FarmReport.newInstance("hisCard.xls")
					.addParameter("result", result.get("rows"))
					.generateForHttp(response, "hisCardReport");
		} catch (Exception e) {
			log.error(e.getMessage());
		}
	}
	...
	public void exportLiveExcel(String ruleText, HttpServletRequest request,
			HttpServletResponse response, HttpSession session) {
		try {
			DataQuery query = new DataQuery();
			query.setPagesize(5000);
			...
			FarmReport.newInstance("liveCard.xls")
					.addParameter("result", result.get("rows"))
					.generateForHttp(response, "liveCardReport");
		} catch (Exception e) {
			log.error(e.getMessage());
		}
	}
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			if (FarmParameterService.getInstance().getParameterBoolean(
					"config.exam.cheat.hiscard.del.able")) {
				for (String id : parseIds(ids)) {
					cardHisServiceImpl.deleteCardhis(id,
