...
public class SubjectUserOwnController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectUserOwnController.class);
	@Resource
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = subjectUserOwnServiceImpl
					.createSubjectuserownSimpleQuery(query).search();
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectUserOwnServiceImpl.deleteSubjectuserownEntity(id,
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								subjectUserOwnServiceImpl
										.getSubjectuserownEntity(ids))
