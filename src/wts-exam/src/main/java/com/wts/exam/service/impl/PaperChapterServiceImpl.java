...
public class PaperChapterServiceImpl implements PaperChapterServiceInter {
	...
	private static final Logger log = Logger
			.getLogger(PaperChapterServiceImpl.class);
	...
	private void initTreeCode(String treeNodeId) {
		...
		if (node.getParentid().equals("NONE")) {
			...
			node.setTreecode(paperchapterDaoImpl.getEntity(node.getParentid())
	...
	public void deletePaperchapterEntity(String id, LoginUser user) {
		// 是否有子节点
		if (paperchapterDaoImpl.selectEntitys(
				DBRuleList.getInstance().add(new DBRule("PARENTID", id, "="))
						.toList()).size() > 0) {
	...
	public void addSubject(String subjectId, String chapterId,
		...
		int num = papersubjectDaoImpl.countEntitys(DBRuleList.getInstance()
				.add(new DBRule("CHAPTERID", chapterId, "=")).toList());
	...
	public boolean isHasSubject(String subjectId, String paperid) {
		List<PaperSubject> sameSubjects = papersubjectDaoImpl
	...
	public void subjectSortUp(String id, LoginUser currentUser) {
		...
		// 查出章节下的所有题目
		List<PaperSubject> relas = papersubjectDaoImpl.selectEntitys(DBRuleList
				.getInstance()
				.add(new DBRule("CHAPTERID", subrela.getChapterid(), "="))
				.toList());
		// 顺序排好// 和上一个交换位置
		int sit1 = 0;
		int sit2 = 0;
	...
	public PaperChapter getEntity(String paperid, String name) {
		List<PaperChapter> list = paperchapterDaoImpl.selectEntitys(DBRuleList
				.getInstance().add(new DBRule("PAPERID", paperid, "="))
				.add(new DBRule("PTYPE", "2", "="))
				.add(new DBRule("NAME", name, "like")).toList());
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
