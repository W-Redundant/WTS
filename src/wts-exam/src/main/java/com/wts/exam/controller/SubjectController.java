...
public class SubjectController extends WebUtils {
	...
	@ResponseBody
	public Map<String, Object> queryall(DataQuery query, String funtype,
			HttpServletRequest request, HttpSession session) {
		try {
			query = EasyUiUtils.formatGridQuery(request, query);
			if (StringUtils.isNotBlank(query.getRule("C.TREECODE"))
					&& query.getRule("C.TREECODE").equals("NONE")) {
				query.getAndRemoveRule("C.TREECODE");
			}
			{
				// 过滤权限
				String typeids_Rule = DataQuerys
						.parseSqlValues(subjectTypeServiceImpl
								.getUserPopTypeids(getCurrentUser(session)
										.getId(), funtype));
			...
			DataResult result = subjectServiceImpl.createSubjectSimpleQuery(
					query).search();
				...
				public void handle(Map<String, Object> row) {
					if (StringUtils.isBlank((String) row.get("TIPSTR"))
						...
						row.put("TIPSTR",
								"[无题目:显示描述]"
										+ HtmlUtils
												.HtmlRemoveTagAndMarkImg((String) row
														.get("TIPNOTE")));
					}
					row.put("TIPSTR", StringEscapeUtils
							.escapeHtml4((String) row.get("TIPSTR")));
				}
			});
	...
	@ResponseBody
	public Map<String, Object> bindMaterial(String subjectIds,
			String materialId, HttpSession session) {
		try {
			for (String id : parseIds(subjectIds)) {
				WcpLog.info("修改题目[" + id + "]:绑定材料", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
				subjectServiceImpl.bindMaterial(id, materialId,
	...
	@ResponseBody
	public Map<String, Object> clearMaterial(String subjectIds,
			HttpSession session) {
		try {
			for (String id : parseIds(subjectIds)) {
				WcpLog.info("修改题目[" + id + "]:清空材料绑定", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Map<String, Object> editSubmit(SubjectVersion entity,
			String tipanalysis, HttpSession session) {
		try {
			WcpLog.info("修改题目[" + entity.getId() + "]:表单提交",
					getCurrentUser(session).getName(), getCurrentUser(session)
							.getId());
			entity = subjectServiceImpl.editSubjectEntity(entity, tipanalysis,
					getCurrentUser(session));
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
	...
	public Map<String, Object> countPoint(String jsons, HttpSession session) {
		try {
			...
			int pointWeight = unit == null ? 0 : cardServiceImpl
					.countSubjectPoint(unit);
			...
			// }
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.putAttr("point", pointWeight).returnObjMode();
		} catch (Exception e) {
			log.error(e.getMessage());
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.setError(e.getMessage(), e).returnObjMode();
	...
	@ResponseBody
	public Map<String, Object> addTextSubjects(String typeid, String texts,
			HttpSession session) {
		try {
			List<SubjectUnit> lists = subjectServiceImpl.addTextSubjects(
					typeid, texts, getCurrentUser(session));
			for (SubjectUnit unit : lists) {
				WcpLog.info("创建题目[" + unit.getSubject().getId() + "]:批量创建",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				subjectServiceImpl
						.refrashAnalysisnum(unit.getSubject().getId());
			}
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.putAttr("lists", lists).returnObjMode();
		} catch (Exception e) {
			log.error(e.getMessage());
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.setError(e.getMessage(), e).returnObjMode();
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				WcpLog.info("刪除题目[" + id + "]:表单刪除", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
				if (!subjectTypeServiceImpl.isHavePop(subjectServiceImpl
						.getSubjectEntity(id).getTypeid(), "2",
						getCurrentUser(session).getId())) {
					throw new RuntimeException("当前用户无该分类题库编辑权限!");
				}
				subjectServiceImpl.deleteSubjectEntity(id,
						getCurrentUser(session));
			}
	...
	public ModelAndView viewLoad(String versionid, HttpSession session) {
		...
		return view.putAttr("subjectu", subjectUnit).returnModelAndView(
				subjectUnit.getTipType().getVeiwPage());
	}
	 ...
	 */
	@RequestMapping("/form")
	public ModelAndView view(RequestMode pageset, String typeid, String ids,
			String tiptype, HttpSession session) {
		try {
			SubjectUnit subjectUnit = null;
			String versionId = null;
			if (StringUtils.isNotBlank(ids)) {
				versionId = subjectServiceImpl.getSubjectEntity(ids)
						.getVersionid();
			}
			...
			case (0): {// 查看
				...
				if (StringUtils.isNotBlank(subjectUnit.getSubject()
						.getMaterialid())) {
					// 如有有引用材料的话f
					view.putAttr("material", materialServiceImpl
							.getMaterialEntity(subjectUnit.getSubject()
									.getMaterialid()));
				}
				if (subjectUnit.getSubject().getAnalysisnum() > 0) {
					// 如有解析的話
					view.putAttr("analysis",
							(List<SubjectAnalysis>) SubjectAnalysisServiceImpl
									.getSubjectAnalysies(subjectUnit
											.getSubject().getId()));
				}
				return view.putAttr("pageset", pageset)
						.putAttr("subjectu", subjectUnit)
						.returnModelAndView("exam/SubjectView");
				// .returnModelAndView(subjectUnit.getTipType().getVeiwPage());
			}
			case (1): {// 新增
				// 1使用权、2维护权
				if (!subjectTypeServiceImpl.isHavePop(typeid, "2",
						getCurrentUser(session).getId())) {
					throw new RuntimeException("当前用户无该分类题库编辑权限!");
				}
				TipType tip = TipType.getTipType(tiptype);
				subjectUnit = subjectServiceImpl.initSubjectUnit(tip, typeid,
						getCurrentUser(session));
				WcpLog.info("创建题目[" + subjectUnit.getSubject().getId()
						+ "]:表单临时创建", getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				break;
			}
			case (2): {// 修改
				subjectUnit = subjectServiceImpl.getSubjectUnit(versionId);
				// 1使用权、2维护权
				if (!subjectTypeServiceImpl.isHavePop(subjectUnit
						.getSubjectType().getId(), "2", getCurrentUser(session)
						.getId())) {
					throw new RuntimeException("当前用户无该分类题库编辑权限!");
				}
				break;
			...
			return ViewMode
					.getInstance()
					.putAttr("operateType", "2")
					.putAttr("subjectu", subjectUnit)
					.returnModelAndView(
							subjectUnit.getTipType().getSubjectPage());
		} catch (Exception e) {
			return ViewMode.getInstance().setError(e.getMessage(), e)
					.returnModelAndView("commons/InnerError");
		}
	}
	...
	@ResponseBody
	public Map<String, Object> examtypeSetting(String ids, String typeId,
			HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				WcpLog.info("修改题目[" + id + "]:移动分类", getCurrentUser(session)
						.getName(), getCurrentUser(session).getId());
				subjectServiceImpl.subjectTypeSetting(id, typeId,
						getCurrentUser(session));
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
