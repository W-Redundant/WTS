 ...
 */
@Repository
public class SubjectAnalysisDaoImpl extends HibernateSQLTools<SubjectAnalysis>
		implements SubjectAnalysisDaoInter {
	...
	public SubjectAnalysis getEntity(String subjectanalysisid) {
		// TODO 自动生成代码,修改后请去除本注释
		Session session = sessionFatory.getCurrentSession();
		return (SubjectAnalysis) session.get(SubjectAnalysis.class,
				subjectanalysisid);
	}
	...
	public List<SubjectAnalysis> getAnalysisByPaperId(String paperId) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select distinct b.ID as ID,b.CTIME as CTIME,b.CUSER as CUSER,b.CUSERNAME as CUSERNAME,b.PSTATE as PSTATE,b.PCONTENT as PCONTENT,b.TEXT as TEXT,b.SUBJECTID as SUBJECTID from WTS_PAPER_SUBJECT a left join WTS_SUBJECT_ANALYSIS b on a.SUBJECTID=b.SUBJECTID where a.paperid=? and b.id is not null");
		sqlquery.setString(0, paperId);
		@SuppressWarnings("unchecked")
		List<SubjectAnalysis> list = (List<SubjectAnalysis>) sqlquery
				.addEntity(SubjectAnalysis.class).list();
		return list;
	}
}
