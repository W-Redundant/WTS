...
public class PaperUserOwnController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(PaperUserOwnController.class);
	@Resource
	...
	public Map<String, Object> queryall(DataQuery query,
		...
		try {
			...
			DataResult result = paperUserOwnServiceImpl
					.createPaperuserownSimpleQuery(query).search();
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				paperUserOwnServiceImpl.deletePaperuserownEntity(id,
						getCurrentUser(session));
			}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								paperUserOwnServiceImpl
										.getPaperuserownEntity(ids))
