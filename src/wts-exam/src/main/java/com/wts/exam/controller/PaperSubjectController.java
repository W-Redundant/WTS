...
public class PaperSubjectController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(PaperSubjectController.class);
	@Resource
	...
	@ResponseBody
	public Map<String, Object> queryall(DataQuery query, String chapterId,
		...
		try {
			...
			DataResult result = paperSubjectServiceImpl
					.createPapersubjectSimpleQuery(query).search();
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Map<String, Object> editSubmit(PaperSubject entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = paperSubjectServiceImpl.editPapersubjectEntity(entity,
	...
	@ResponseBody
	public Map<String, Object> editPoint(String paperSubjectIds, Integer point,
			HttpSession session) {
		try {
			for (String id : parseIds(paperSubjectIds)) {
				PaperSubject psub = paperSubjectServiceImpl
						.getPapersubjectEntity(id);
				WcpLog.info("修改答卷[" + psub.getPaperid() + "]:设置题目得分",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				paperServiceImpl.editPoint(psub.getPaperid(),
						psub.getChapterid(), psub.getSubjectid(), point,
						getCurrentUser(session));
			}
			return ViewMode.getInstance().setOperate(OperateType.UPDATE)
					.returnObjMode();
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(PaperSubject entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = paperSubjectServiceImpl.insertPapersubjectEntity(entity,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			...
			for (String id : parseIds(ids)) {
				paperId = paperSubjectServiceImpl.getPapersubjectEntity(id)
						.getPaperid();
				WcpLog.info("修改答卷[" + paperId + "]:移除题目",
						getCurrentUser(session).getName(),
						getCurrentUser(session).getId());
				paperSubjectServiceImpl.deletePapersubjectEntity(id,
						getCurrentUser(session));
			}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								paperSubjectServiceImpl
										.getPapersubjectEntity(ids))
