...
public class LateXController extends WebUtils {
	...
	public ModelAndView index(HttpSession session) {
		List<Entry<String, String>> types = FarmParameterService.getInstance()
				.getDictionaryList("LATEX_MODE");
		return ViewMode.getInstance().putAttr("types", types)
				.returnModelAndView("exam/comment/latexInput");
	}
	...
	@ResponseBody
	public Map<String, Object> valid(String latex, HttpServletRequest request,
			HttpSession session) {
		...
		return ViewMode.getInstance().putAttr("error", error)
				.putAttr("message", message).returnObjMode();
	}
	 ...
	 */
	@RequestMapping("/view")
	public void loadimg(String latex, HttpServletRequest request,
			HttpServletResponse response) {
		latex = StringEscapeUtils.unescapeHtml4(latex);
		FileDownloadUtils.simpleDownloadFile(LatexUtils.latex2Bytes(latex),
				"科学公式.png", "application/octet-stream", response);
	}
	...
	@ResponseBody
	public Map<String, Object> downLoadLateXpng(String latex,
		...
		try {
			...
			id = farmFileManagerImpl
					.saveFile(LatexUtils.latex2Bytes(latex),
							FILE_TYPE.RESOURCE_IMG, "科学公式.png",
		...
		return ViewMode.getInstance().putAttr("error", error)
				.putAttr("url", url).putAttr("message", message)
				.putAttr("id", id).putAttr("fileName", fileName)
				.returnObjMode();
	}
}
