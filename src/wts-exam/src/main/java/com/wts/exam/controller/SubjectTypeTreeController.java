...
public class SubjectTypeTreeController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectTypeTreeController.class);
	@Resource
	...
	public Object subjecttypeTree(String id, String funtype, HttpSession session) {
		try {
			if (id == null) {
				...
				EasyUiTreeNode nodes = new EasyUiTreeNode("NONE", "题库分类",
						"open", "icon-customers");
				nodes.setChildren(EasyUiTreeNode.formatAsyncAjaxTree(
						EasyUiTreeNode.queryTreeNodeOne(id, "SORT",
								"WTS_SUBJECT_TYPE", "ID", "PARENTID", "NAME",
								"CTIME").getResultList(),
						EasyUiTreeNode.queryTreeNodeTow(id, "SORT",
								"WTS_SUBJECT_TYPE", "ID", "PARENTID", "NAME",
								"CTIME").getResultList(), "PARENTID", "ID",
						"NAME", "CTIME"));
				list.add(nodes);
				list = subjectTypeServiceImpl.RunPopFilter(list, funtype,
						getCurrentUser(session));
				return list;
			} else {
				List<EasyUiTreeNode> list = EasyUiTreeNode.formatAsyncAjaxTree(
						EasyUiTreeNode.queryTreeNodeOne(id, "SORT",
								"WTS_SUBJECT_TYPE", "ID", "PARENTID", "NAME",
								"CTIME").getResultList(),
						EasyUiTreeNode.queryTreeNodeTow(id, "SORT",
