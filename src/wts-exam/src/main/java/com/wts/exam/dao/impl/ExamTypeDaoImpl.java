 ...
 */
@Repository
public class ExamTypeDaoImpl extends HibernateSQLTools<ExamType> implements
		ExamTypeDaoInter {
	...
	public int getAllListNum() {
		...
		SQLQuery sqlquery = session
				.createSQLQuery("select count(*) from WTS_EXAM_TYPE");
	...
	public List<ExamType> getAllSubNodes(String orgId) {
		...
		Query sqlquery = session
				.createQuery("from ExamType where TREECODE like ? order by ctime ");
	...
	public List<String> getAllSubType(List<String> typeIds) {
		...
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select ID from WTS_EXAM_TYPE where "
						+ whereSql);
		return sqlquery.list();
	}
	...
	public Set<String> getUserPopTypeids(String userId, String... funtypes) {
		...
		for (String fun : funtypes) {
			...
			if (fun.equals("1")) {
				where = where
						+ " OR a.MNGPOP = '1' OR ( b.FUNTYPE = '1' AND b.USERID = '"
						+ userId + "' )";
			}
			// 2:判卷权限.
			if (fun.equals("2")) {
				where = where
						+ " OR a.ADJUDGEPOP = '1' OR ( b.FUNTYPE = '2' AND b.USERID = '"
						+ userId + "' )";
			}
			// 3:查询权限.
			if (fun.equals("3")) {
				where = where
						+ " OR a.QUERYPOP = '1' OR ( b.FUNTYPE = '3' AND b.USERID = '"
						+ userId + "' )";
			}
			// 4:超级权限
			if (fun.equals("4")) {
				where = where
						+ " OR a.SUPERPOP = '1' OR ( b.FUNTYPE = '4' AND b.USERID = '"
						+ userId + "' )";
			}
		}
	...
	public List<Map<String, Object>> getTypePopUsers(String examtypeid,
		...
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("SELECT A.ID AS TYPEID, A. NAME AS TYPENAME, B.USERID AS USERID, C. NAME AS USERNAME, E.ID AS ORGID, E. NAME AS ORGNAME "
