...
public class SubjecTanswerController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjecTanswerController.class);
	@Resource
	...
	@ResponseBody
	public Map<String, Object> queryall(DataQuery query, String versionid,
		...
		try {
			...
			DataResult result = subjectAnswerServiceImpl
					.createSubjectanswerSimpleQuery(query).search();
				...
				public void handle(Map<String, Object> row) {
					if (StringUtils.isBlank((String) row.get("ANSWER"))
							&& StringUtils.isNotBlank((String) row
									.get("ANSWERNOTE"))) {
						row.put("ANSWER", HtmlUtils
								.HtmlRemoveTagAndMarkImg((String) row
										.get("ANSWERNOTE")));
					}
					row.put("ANSWER", StringEscapeUtils
							.escapeHtml4((String) row.get("ANSWER")));
				}
			});
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Map<String, Object> editSubmit(SubjectAnswer entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = subjectAnswerServiceImpl.editSubjectanswerEntity(entity,
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(SubjectAnswer entity,
			HttpSession session) {
		// TODO 自动生成代码,修改后请去除本注释
		try {
			entity = subjectAnswerServiceImpl.insertSubjectanswerEntity(entity,
					getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectAnswerServiceImpl.deleteSubjectanswerEntity(id,
						getCurrentUser(session));
			}
	...
	public Map<String, Object> onlyRight(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectAnswerServiceImpl.setOnlyRight(id,
						getCurrentUser(session));
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	@ResponseBody
	public Map<String, Object> rightVal(String flag, String ids,
			HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				subjectAnswerServiceImpl.setRightAnswer(id, flag.equals("1"),
						getCurrentUser(session));
			}
			return ViewMode.getInstance().returnObjMode();
		} catch (Exception e) {
	...
	public ModelAndView view(RequestMode pageset, String ids, String versionid) {
		try {
			...
			case (0): {// 查看
				SubjectAnswer answer = subjectAnswerServiceImpl
						.getSubjectanswerEntity(ids);
				SubjectVersion version = subjectVersionServiceImpl
						.getSubjectversionEntity(answer.getVersionid());
				TipType tipType = TipType.getTipType(version.getTiptype());
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("entity", answer)
						.returnModelAndView(tipType.getAnswerPage());
			}
			case (1): {// 新增
				SubjectVersion version = subjectVersionServiceImpl
						.getSubjectversionEntity(versionid);
				TipType tipType = TipType.getTipType(version.getTiptype());
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("versionid", versionid)
						.returnModelAndView(tipType.getAnswerPage());
			}
			case (2): {// 修改
				SubjectAnswer answer = subjectAnswerServiceImpl
			...
			return ViewMode.getInstance().returnModelAndView(
					"exam/SubjectViews/VacancyAnswer");
		} catch (Exception e) {
			return ViewMode.getInstance().setError(e + e.getMessage(), e)
					.returnModelAndView("exam/SubjectanswerForm");
		}
	}
}
