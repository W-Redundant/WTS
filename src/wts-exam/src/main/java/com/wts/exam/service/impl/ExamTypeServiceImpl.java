...
public class ExamTypeServiceImpl implements ExamTypeServiceInter {
	...
	private static final Logger log = Logger
			.getLogger(ExamTypeServiceImpl.class);
	...
	private void initTreeCode(String treeNodeId) {
		...
		if (node.getParentid().equals("NONE")) {
			...
			node.setTreecode(examtypeDaoImpl.getEntity(node.getParentid())
	...
	public void deleteExamtypeEntity(String id, LoginUser user) {
		{
			List<ExamType> types = examtypeDaoImpl.selectEntitys(DBRuleList
					.getInstance().add(new DBRule("PARENTID", id, "="))
					.toList());
			if (types.size() > 0) {
		...
		{
			List<Paper> papers = paperDaoImpl.selectEntitys(DBRuleList
					.getInstance().add(new DBRule("EXAMTYPEID", id, "="))
					.toList());
			if (papers.size() > 0) {
		...
		{
			List<Room> rooms = roomDaoImpl.selectEntitys(DBRuleList
					.getInstance().add(new DBRule("EXAMTYPEID", id, "="))
					.toList());
			if (rooms.size() > 0) {
		...
		for (ExamType type : types) {
			...
			try {
				...
				for (Room room : rooms) {
					...
					ExamType examtype = examtypeDaoImpl.getEntity(room
							.getExamtypeid());
