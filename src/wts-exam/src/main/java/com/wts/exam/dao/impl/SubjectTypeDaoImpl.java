 ...
 */
@Repository
public class SubjectTypeDaoImpl extends HibernateSQLTools<SubjectType>
		implements SubjectTypeDaoInter {
	...
	public List<SubjectType> getAllSubNodes(String orgId) {
		...
		Query sqlquery = session
				.createQuery("from SubjectType where TREECODE like ? order by ctime ");
	...
	public List<String> getAllSubType(List<String> typeIds) {
		...
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select ID from WTS_SUBJECT_TYPE where "
						+ whereSql);
		return sqlquery.list();
	}
	...
	public Set<String> getUserPopTypeids(String userId, String funtype) {
		...
		if (funtype.equals("1")) {
			sql = "SELECT DISTINCT a.id FROM WTS_SUBJECT_TYPE a LEFT JOIN WTS_SUBJECT_POP b ON b.TYPEID = a.ID WHERE "
					+ " a.READPOP = '1' OR ( b.FUNTYPE = '1' AND b.USERID = '"
					+ userId + "' )";
		}
		// 2维护权
		if (funtype.equals("2")) {
			sql = "SELECT DISTINCT a.id FROM WTS_SUBJECT_TYPE a LEFT JOIN WTS_SUBJECT_POP b ON b.TYPEID = a.ID WHERE "
					+ " a.WRITEPOP = '1' OR ( b.FUNTYPE = '2' AND b.USERID = '"
					+ userId + "' )";
		}
