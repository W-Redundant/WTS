...
public class SubjectAnalysisController extends WebUtils {
	private final static Logger log = Logger
			.getLogger(SubjectAnalysisController.class);
	@Resource
	...
	@ResponseBody
	public Map<String, Object> queryall(String subjectid, DataQuery query,
			HttpServletRequest request) {
		try {
			query = EasyUiUtils.formatGridQuery(request, query);
			query.addRule(new DBRule("subjectid", subjectid, "="));
			DataResult result = SubjectAnalysisServiceImpl
					.createSubjectAnalysisSimpleQuery(query).search();
	...
	@RequestMapping("/edit")
	@ResponseBody
	public Map<String, Object> editSubmit(SubjectAnalysis entity,
			HttpSession session) {
		try {
			entity = SubjectAnalysisServiceImpl.editSubjectAnalysisEntity(
					entity, getCurrentUser(session));
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addSubmit(SubjectAnalysis entity,
			HttpSession session) {
		try {
			entity = SubjectAnalysisServiceImpl.insertSubjectAnalysisEntity(
					entity, getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				String subjectid = SubjectAnalysisServiceImpl
						.getSubjectAnalysisEntity(id).getSubjectid();
				SubjectAnalysisServiceImpl.deleteSubjectAnalysisEntity(id,
	...
	public ModelAndView index(String subjectid, HttpSession session) {
		return ViewMode.getInstance().putAttr("subjectid", subjectid)
				.returnModelAndView("exam/SubjectanalysisResult");
	}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode
						...
						.putAttr(
								"entity",
								SubjectAnalysisServiceImpl
										.getSubjectAnalysisEntity(ids))
						.returnModelAndView("exam/SubjectanalysisForm");
			}
			case (1): {// 新增
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("subjectid", subjectid)
						.returnModelAndView("exam/SubjectanalysisForm");
			}
			case (2): {// 修改
				return ViewMode
