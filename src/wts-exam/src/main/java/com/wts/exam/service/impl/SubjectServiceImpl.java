...
public class SubjectServiceImpl implements SubjectServiceInter {
	...
	public void deleteSubjectEntity(String subjectId, LoginUser user) {
		...
		// 删除引用
		DBRuleList dbRuleList = DBRuleList.getInstance().add(
				new DBRule("SUBJECTID", subjectId, "="));
	...
	public SubjectUnit getSubjectUnit(String versionId) {
		...
		unit.setSubject(subjectDaoImpl.getEntity(unit.getVersion()
				.getSubjectid()));
		unit.setSubjectType(subjecttypeDaoImpl.getEntity(unit.getSubject()
				.getTypeid()));
	...
	public int clearSubject(LoginUser currentUser) {
		List<Subject> subjects = subjectDaoImpl.selectEntitys(DBRuleList
				.getInstance().add(new DBRule("PSTATE", "0", "=")).toList());
	...
	public int refrashAnalysisnum(String subjectid) {
		int num = SubjectAnalysisDaoImpl.countEntitys(DBRuleList.getInstance()
				.add(new DBRule("subjectid", subjectid, "=")).toList());
		Subject subject = subjectDaoImpl.getEntity(subjectid);
	...
	public int refrashCommentnum(String subjectid) {
		int num = SubjectCommentDaoImpl.countEntitys(DBRuleList.getInstance()
				.add(new DBRule("subjectid", subjectid, "=")).toList());
		Subject subject = subjectDaoImpl.getEntity(subjectid);
