...
import com.farm.authority.mapper.ActiontreeMapper;
...
public class ActionServiceImpl implements ActionServiceInter {
    ...
    public Action insertActionEntity(Action entity, LoginUser user) {
        ...
        if (actionMapper.getEntityByKey(entity.getAuthkey()) != null) {
    ...
    public Action editActionEntity(Action entity, LoginUser user) {
        Action entity2 = actionMapper.selectByPrimaryKey(entity.getId());
        ...
        actionMapper.updateByPrimaryKeySelective(entity2);
        Action keyAction = actionMapper.getEntityByKey(entity.getAuthkey());
    ...
    public Actiontree insertActiontreeEntity(Actiontree entity, LoginUser user, String authkey) {
        ...
        if (entity.getType().equals("1")) {
            ...
            if (entity.getActionid() == null || entity.getActionid().trim().length() <= 0) {
                ...
                if (!entity.getParentid().equals("NONE")) {
                    actionName = actiontreeMapper.selectByParentId(entity.getParentid()).getName() + '_' + entity.getName();
                } else {
        ...
        actiontreeMapper.insert(entity);
    ...
    public Actiontree editActiontreeEntity(Actiontree entity, LoginUser user, String authkey) {
        Actiontree entity2 = actiontreeMapper.selectByPrimaryKey(entity.getId());
        ...
        actiontreeMapper.updateByPrimaryKeySelective(entity2);
        return entity2;
    }
    ...
    public void deleteActiontreeEntity(String id, LoginUser user) {
        ...
        if (actiontreeMapper.selectEntitys(id).size() > 0) {
            throw new RuntimeException("不能删除该节点，请先删除其子节点");
        }
        actiontreeMapper.deleteByPrimaryKey(id);
    }
    ...
    public Actiontree getActiontreeEntity(String id) {
        ...
        return actiontreeMapper.selectByPrimaryKey(id);
    }
    ...
    public AuthKey getCacheAction(String key) {
        if (keyMap.size() <= 0) {
            List<Action> list = actionMapper.selectAll();
